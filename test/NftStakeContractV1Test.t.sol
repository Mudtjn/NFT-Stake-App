...
contract NftStakeContractV1Test is Test {
    ...
    function testClaimRewardsUpdatesUserBalanceAndInformation() public {
        ...
        vm.warp(block.timestamp + NftStakeContractV1(proxy).getMinDelayBetweenRewards());
        vm.roll(10); 
    ...
    function testAlreadyUnstakedNftsCannotBeUnstakedAgain() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(NftStakeContractV1.NftStakeContractV1__NftAlreadyUnstaked.selector, vaultTokenId)); 
        NftStakeContractV1(proxy).unstakeNft(vaultTokenId);
        vm.stopPrank(); 
    }
    ...
    function testWithdrawNftFailsForStakedNfts() public {
        ...
        vm.startPrank(user); 
        uint256 vaultTokenId = NftStakeContractV1(proxy).stakeNft(address(nftMock), tokenId);
        vm.expectRevert(abi.encodeWithSelector(NftStakeContractV1.NftStakeContractV1__NftStillStaked.selector, vaultTokenId)); 
        NftStakeContractV1(proxy).withdrawNft(vaultTokenId); 
        vm.stopPrank(); 
    }    
    ...
    function testWithdrawNftUpdatesInformationCorrectly() public {
        ...
        vm.warp(block.timestamp + NftStakeContractV1(proxy).getUnbondingPeriod());
        vm.roll(1); 
        vm.prank(user); 
        NftStakeContractV1(proxy).withdrawNft(vaultTokenId);
